...
def logtime(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        end_time = time.time()
        logging.info(f"ITERATION: {SteSerial.iter}, {func.__name__} executed in {end_time - start_time:.4f} seconds")
        return result
    return wrapper
...
class SteSerial:
    ...
    def run(self):
        ...
        SteSerial.iter = 0
        self.error = self.max_error + 1
        while SteSerial.iter < self.max_steps:
            start_time = time.time()
            self.step()
            ...
            print(f'\n\nITER: {SteSerial.iter}, ERROR: {self.error}, LOWEST_ENERGY: {self.ste_eigs.real.min()}, STEP_TIME: {elapsed_time}\n\n', flush=True)
            if self.error < self.max_error:
                break
            else:
                SteSerial.iter += 1
